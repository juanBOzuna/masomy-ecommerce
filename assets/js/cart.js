...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var containerCountableDetails = document.querySelector(
    "#container_countable_details"
  );
  ...
  function actProduct(existingProduct, action = "sum") {
    const alterCountableDetail = {
      subtotal: 0,
      total: 0,
      totalDiscountAmmount: 0,
    };
    let cartCountableDetails =
      JSON.parse(localStorage.getItem("cart_countable_details")) || {};

    if (
      cartCountableDetails.total == null ||
      cartCountableDetails.subtotal == null ||
      cartCountableDetails.totalDiscountAmmount == null
    ) {
    ...
    if (!valDelete) {
      ...
      console.log(
        "suubt" +
          (subtotal * parseFloat(existingProduct.discountPercentaje)) / 100
      );
      let discount =
        (subtotal * parseFloat(existingProduct.discountPercentaje)) / 100;
      ...
      existingProduct.discountPercentaje = parseFloat(
        existingProduct.discountPercentaje
      );
    ...
    localStorage.setItem(
      "cart_countable_details",
      JSON.stringify(cartCountableDetails)
    );
  ...
  function renderProducts() {
    ...
    if (cartProducts.length != 0) {
      ...
      document.querySelector("#totalCompra").innerText =
        "TOTAL: " + cartCountableDetails.total;
      document.querySelector("#descuentoCompra").innerText =
        "DESCUENTO: " + cartCountableDetails.totalDiscountAmmount;
      document.querySelector("#subtotalCompra").innerText =
        "SUBTOTAL: " + cartCountableDetails.subtotal;
    ...
    if (accessToken != "") {
      ...
      fetch(`${serverUrl}/api/generatePayMentLink`, requestOptions)
        ...
        .then((result) => {
          ...
          document.getElementById("payment-reference-container").style.display =
            "block";
          // document.getElementById("payment-reference-text").innerText = "Referencia de Pago: " + result.payment_reference;
          document.getElementById("reference-code").innerText =
